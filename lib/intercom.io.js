 ...
 * @api public
 */
function Intercom(appId, options) {
  // Overload the contractor
  // Parse out single option objects
  if (_.isObject(appId) && !_.isString(appId) && appId.appId) {
    options = _.omit(appId, 'appId');
    appId = appId.appId;
  }
  ...
  // Contruct the endpoint with the correct auth from the appId
 ...
 * @api public
 */
Intercom.create = function(appId, options) {
  var intercom = new Intercom(appId, options);
  return intercom;
};
